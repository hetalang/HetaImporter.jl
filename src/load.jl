...
function load_platform(
  ...
  export_format = isnothing(spaceFilter) ? "{format:Julia, filepath:$MODEL_DIR}" : "{format:Julia, filepath:$MODEL_DIR, spaceFilter:'$spaceFilter'}"
  # convert heta model to julia
  dist_dir = rm_out ? mktempdir() : abspath(target_dir) # absolute
  build_res = heta_build(target_dir; dist_dir, export_format , kwargs...)
